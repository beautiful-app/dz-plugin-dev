...
updatemembercount($_G['uid'], array($misign['nrcredit'] => $ajaxcreditshow['normal']['number']));
...
if(memory('check')) memory('set', 'k_misign_'.$uid, $_G['timestamp'], 86400);
if($isfirst || $row == '1') {
...
if($misign['qdtype'] == '2') {
	...
	if($num >= 0 && ($row <= $extreward_num) && $exacr && $exacz) {
		$message = lang('plugin/k_misign', 'tsn_extrac', array('time' => $hft, 'row' => $row, 'extrac' => $_G['setting']['extcredits'][$misign['nrcredit']]['title'].' '.$credit.' '.$_G['setting']['extcredits'][$misign['nrcredit']]['unit'], 'extrac2' => $_G['setting']['extcredits'][$exacr]['title'].$exacz.$_G['setting']['extcredits'][$exacr]['unit']));
	} else {
		$message = lang('plugin/k_misign', 'tsn_noextrac', array('time' => $hft, 'extrac' => $_G['setting']['extcredits'][$misign['nrcredit']]['title'].' '.$credit.' '.$_G['setting']['extcredits'][$misign['nrcredit']]['unit']));
	}
	...
	if($isfirst || $stats['qdtidnumber'] == '0') {
		...
		if($exacr && $exacz) {
			$message = lang('plugin/k_misign', 'tsn_thread').lang('plugin/k_misign', 'tsn_extrac', array('time' => $hft, 'row' => $row, 'extrac' => $_G['setting']['extcredits'][$misign['nrcredit']]['title'].' '.$credit.' '.$_G['setting']['extcredits'][$misign['nrcredit']]['unit'], 'extrac2' => $_G['setting']['extcredits'][$exacr]['title'].$exacz.$_G['setting']['extcredits'][$exacr]['unit']));
		} else {
			$message = lang('plugin/k_misign', 'tsn_thread').lang('plugin/k_misign', 'tsn_noextrac', array('time' => $hft, 'extrac' => $_G['setting']['extcredits'][$misign['nrcredit']]['title'].' '.$credit.' '.$_G['setting']['extcredits'][$misign['nrcredit']]['unit']));
		}
        $thread_param = array(
			'isgroup' => '0',
			'status' => '0',
...
if($qiandaodb['mdays'] < dgmdate($_G['timestamp'], 'j', $misign['tos'])){
	...
	C::t("#k_misign#plugin_k_misign")->update_signdata_outmonth($_G['uid'], $credit, $row, $islasted);
}
