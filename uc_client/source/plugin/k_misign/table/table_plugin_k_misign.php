...
class table_plugin_k_misign extends discuz_table{
	...
	public function update_signdata_outmonth($uid, $credit, $row, $islasted) {
        if($islasted){
            return DB::query('UPDATE %t SET time=%i,reward=reward+%i,lastreward=%i,row=%d,lasted=lasted+1 WHERE uid=%d', array($this->_table, TIMESTAMP, $credit, $credit, $row, $uid));
        }else{
            return DB::query('UPDATE %t SET time=%i,reward=reward+%i,lastreward=%i,row=%d,lasted=1 WHERE uid=%d', array($this->_table, TIMESTAMP, $credit, $credit, $row, $uid));
        }
    }
